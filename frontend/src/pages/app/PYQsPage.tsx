...
export function PYQsPage() {
  ...
  useEffect(() => {
    listQuestions({ pyq_only: true })
      .then(r => setItems(r.data || []))
      .catch(e => setError(e?.message || "Could not load PYQs"));
  }, []);

  const years = [...new Set(items.map(q => q.year).filter((y: any) => y != null))].sort((a: any, b: any) => b - a);

  const filtered = items.filter(q => {
    if (filterSubject !== "all" && q.subjectId !== filterSubject) return false;
    if (filterYear !== "all" && String(q.year) !== filterYear) return false;
    if (filterDifficulty !== "all" && String(q.difficulty || "").toLowerCase() !== filterDifficulty) return false;
    return true;
  });
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">Extracted from uploaded question papers. Years and marks are shown only when present in the source.</p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white/3 border border-white/6 rounded-2xl p-14 text-center">
          ...
          <p className="text-slate-400">No extracted PYQs yet. Upload a question paper in My Library.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((pyq, i) => {
            const sub = SUBJECTS.find(s => s.id === pyq.subjectId);
            const expanded = expandedId === String(pyq.id);
            return (
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex flex-wrap gap-2">
                      {pyq.year != null && <span className="text-xs bg-indigo-500/15 text-indigo-300 border border-indigo-500/20 px-2 py-0.5 rounded-lg">{pyq.year}</span>}
                      {pyq.marks != null && <span className="text-xs bg-white/8 text-slate-400 px-2 py-0.5 rounded-lg">{pyq.marks} marks</span>}
                      {pyq.difficulty && <span className="text-xs px-2 py-0.5 rounded-lg bg-white/8 text-slate-400">{String(pyq.difficulty).toLowerCase()}</span>}
                      <span className="text-xs px-2 py-0.5 rounded-lg bg-white/8 text-slate-500">{pyq.source}</span>
                  ...
                  {pyq.pageNumber != null && <div className="text-xs text-slate-600 mt-2">Page {pyq.pageNumber}</div>}
                </div>

                {(pyq.answer || pyq.explanation) && (
                <div className="border-t border-white/6">
                  <button onClick={() => setExpandedId(expanded ? null : String(pyq.id))}
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="overflow-hidden">
                      <div className="px-5 pb-5 space-y-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-slate-300 leading-relaxed bg-black/20 p-3 rounded-xl">{pyq.answer}</div>
                        </div>
                        )}
                        {pyq.explanation && (
                        <div>
