...
import { getLearningPath, getMastery, masteryColor } from "../../services/adaptiveEngine";
...
export function LearnPage() {
  ...
  const available = path.filter(i => i.status !== "locked");
            ...
            <motion.div key={item.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}>
              <Link to={`/app/concepts/${concept.id}`}>
                <div className="bg-white/3 border border-white/6 rounded-2xl p-5 hover:bg-white/5 transition-all group flex items-center gap-4">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${item.status === "mastered" ? "bg-emerald-500/20 text-emerald-400" : "bg-white/5 text-slate-400"}`}>
                    {item.status === "mastered" ? <CheckCircle size={16} /> : <span className="text-xs font-bold">{item.order}</span>}
                  </div>
                  ...
                  {item.mastery > 0 && <div className={`text-sm font-bold shrink-0 ${masteryColor(item.mastery)}`}>{item.mastery}%</div>}
